...
class DbClassOrder implements DbClass {
	...
	void submitOrder(ShoppingCart shopCart) throws DatabaseException {
		...
		order = OrderSubsystemFacade.createOrder(orderId, LocalDate.now(), orderItems);
	...
	private void buildGetOrderDataQuery() {
		query = "SELECT orderid, orderdate, totalpriceamount FROM Ord WHERE orderid = "
				+ orderId;
	}
	...
	private void populateOrderItems(ResultSet rs) throws DatabaseException,
			BackendException, SQLException {
		orderItems = new LinkedList<OrderItem>();
			while (rs.next()) {
				orderItem = OrderSubsystemFacade.createOrderItem(
						rs.getInt("productid"), rs.getInt("orderid"),
	...
	private void populateOrderData(ResultSet resultSet)
			throws DatabaseException, BackendException {
	...
	public void populateEntity(ResultSet resultSet) throws DatabaseException {
		if (queryType.equals(GET_ORDER_ITEMS)) {
			...
			} catch (BackendException | SQLException e) {
				e.printStackTrace();
			}

...
public class SelectAddress extends Stage implements MessageableWindow {
	...
	public SelectAddress(ShippingBillingWindow w, CheckoutData.ShipBill shipBill, 
      ...
      //add buttons
  		...
  		/*ObservableList<CartItemPres> tableItems = BrowseSelectData.INSTANCE.getCartData();
		    ObservableList<CartItemPres> selectedItems = table.getSelectionModel().getSelectedItems();
		    ...
		    if(tableItems.isEmpty()) {
		    	...
		    	boolean result = BrowseSelectData.INSTANCE.removeFromCart(selectedItems);
			    if(result) {
			    	table.setItems(BrowseSelectData.INSTANCE.getCartData());
			    	setTotalInCart(GuiUtils.formatPrice(GuiUtils.computeTotalInTable(table)));
			    	clearMessages();
			    } else {
			    	displayInfo("Zero items removed from cart.");
			    }
		    }*/
